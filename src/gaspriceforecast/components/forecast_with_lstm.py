...
from gaspriceforecast.entity.config_entity import ForecastWithLSTMConfig
from gaspriceforecast.utils.logger import get_logger

logger = get_logger("forecast_with_lstm.log")


class ForecastWithLSTM:
    def __init__(self, config: ForecastWithLSTMConfig):
        self.config = config
    ...
    def forecast(self, features_df):
        logger.info("Running LSTM forecast on future data...")
    ...
    def plot_and_save(self, preds):
        ...
        plt.title("Next 30-Day LSTM Forecast")
        plt.xlabel("Date")
